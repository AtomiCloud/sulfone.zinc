...
public class ProcessorService(IProcessorRepository repo, ILogger<ProcessorService> logger, IUserRepository user)
  : IProcessorService
{
      ...
      return await repo.GetVersion(username, name, version)
        .DoAwait(DoType.Ignore, async _ =>
        {
          ...
          if (r.IsFailure())
          {
            ...
            logger.LogError(err, "Failed to increment download when obtaining version for Processor '{Username}/{Name}:{Version}'",
    ...
    if (bumpDownload)
    {
      return await repo.GetVersion(username, name)
        .DoAwait(DoType.Ignore, async _ =>
        {
          ...
          {
            var err = r.FailureOrDefault();
            logger.LogError(err, "Failed to increment download when obtaining version for Processor '{Username}/{Name}'",
              username, name);
          }
