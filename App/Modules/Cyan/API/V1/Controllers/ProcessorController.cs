...
{
  [HttpGet]
  public async Task<ActionResult<IEnumerable<ProcessorPrincipalResp>>> Search(
    [FromQuery] SearchProcessorQuery query
  )
  {
  ...
  {
    var processor = await service.Get(userId, processorId).Then(x => x?.ToResp(), Errors.MapAll);
  ...
  {
    var processor = await service.Get(username, name).Then(x => x?.ToResp(), Errors.MapAll);
    return this.ReturnNullableResult(
      processor,
  ...
  public async Task<ActionResult<ProcessorPrincipalResp>> Create(
    string userId,
    [FromBody] CreateProcessorReq req
  )
  {
    var sub = this.Sub();
    if (sub != userId)
    {
      Result<ProcessorPrincipalResp> e = new Unauthorized(
        "You are not authorized to create a processor for this user"
      ).ToException();
  ...
  public async Task<ActionResult<ProcessorPrincipalResp>> Update(
    string userId,
    Guid processorId,
  ...
  )
  {
    ...
    if (sub != likerId)
    {
      Result<Unit> e = new Unauthorized(
        "You are not authorized to like this processor"
      ).ToException();
      return this.ReturnResult(e);
    }

    var processor = await service
      .Like(likerId, username, processorName, like)
      .Then(x => x.ToResult());
  ...
  {
    var processor = await service.Delete(userId, processorId).Then(x => x.ToResult());
    return this.ReturnUnitNullableResult(
      processor,
      new EntityNotFound(
        "Processor not found",
  ...
  public async Task<ActionResult<IEnumerable<ProcessorVersionPrincipalResp>>> SearchVersion(
    string username,
    string processorName,
  ...
  public async Task<ActionResult<ProcessorVersionResp>> GetVersion(
    string username,
    string processorName,
    ulong ver,
    bool bumpDownload
  ...
  {
    var processor = await service
      .GetVersion(username, processorName, ver, bumpDownload)
      .Then(x => x?.ToResp(), Errors.MapAll);
    return this.ReturnNullableResult(
      processor,
      new EntityNotFound(
  ...
  {
    var processor = await service
      .GetVersion(username, processorName, bumpDownload)
      .Then(x => x?.ToResp(), Errors.MapAll);
    return this.ReturnNullableResult(
      processor,
      new EntityNotFound(
  ...
  {
    var processor = await service
      .GetVersion(userId, processorId, ver)
      .Then(x => x?.ToResp(), Errors.MapAll);
    return this.ReturnNullableResult(
      processor,
      new EntityNotFound(
        "Processor not found",
        typeof(ProcessorVersion),
        $"{userId}/{processorId}:{ver}"
      )
    );
  }

  [Authorize, HttpPost("slug/{username}/{processorName}/versions")]
  public async Task<ActionResult<ProcessorVersionPrincipalResp>> CreateVersion(
    string username,
    string processorName,
      ...
      .ThenAwait(async x =>
      {
        if (x)
        {
          return await createProcessorVersionReqValidator
            ...
            .ThenAwait(c =>
              service.CreateVersion(username, processorName, c.ToDomain().Item2, c.ToDomain().Item1)
            )
  ...
  )
  {
    ...
    if (sub != userId)
    {
      Result<ProcessorVersionPrincipalResp> e = new Unauthorized(
        "You are not authorized to create a processor for this user"
      ).ToException();
      return this.ReturnResult(e);
    }
    ...
    var version = await createProcessorVersionReqValidator
      .ValidateAsyncResult(req, "Invalid CreateProcessorVersionReq")
      .ThenAwait(x =>
        service.CreateVersion(userId, processorId, x.ToDomain().Item2, x.ToDomain().Item1)
      )
      .Then(x => x?.ToResp(), Errors.MapAll);
  ...
  public async Task<ActionResult<ProcessorVersionPrincipalResp>> UpdateVersion(
    string userId,
    Guid processorId,
    ulong ver,
