...
public class FileValidator(ContentInspector inspector, ILogger<FileValidator> logger)
  : IFileValidator
{
  ...
  public Task<Result<Stream>> Validate(Stream stream, FileValidationParam param)
  {
    ...
    // Validate file
    var d = inspector.Inspect(stream).MaxBy(x => x.Points)?.Definition;
    if (d == null)
    {
    ...
    {
      var err = new InvalidFileType(
        $"File must be one of '{param.AllowedMime.Humanize()}' but was '{mime}'",
        mime,
        param.AllowedMime
      );
      logger.LogError(
        err.ToException(),
        "File must be one of '{Allowed}' but was '{Mime}'",
        param.AllowedMime.Humanize(),
        mime
      );
      return Task.FromResult<Result<Stream>>(err.ToException());
    }
    ...
    if (param.AllowedExt != null && !param.AllowedExt.Any(x => d.File.Extensions.Contains(x)))
    {
      var err = new InvalidFileExt(
        $"File must be one of '{param.AllowedExt.Humanize()}' but was '{d.File.Extensions.Humanize()}'",
        ext,
        param.AllowedExt
      );
      logger.LogError(
        err.ToException(),
        "File must be one of '{Allowed}' but was '{Extension}'",
        param.AllowedExt.Humanize(),
        d.File.Extensions.Humanize()
      );
      return Task.FromResult<Result<Stream>>(err.ToException());
    }
    ...
    if (param.MaxSize != null && stream.Length > param.MaxSize)
    {
      ...
      logger.LogError(
        ...
        maxSize,
        maxSize.Bits().Humanize(),
        stream.Length,
        stream.Length.Bits().Humanize()
      );
      return Task.FromResult<Result<Stream>>(err.ToException());
    }
